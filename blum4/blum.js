...
function getRandomPoints() {
  return Math.floor(Math.random() * (200 - 180 + 1)) + 180;
}
...
function isTokenExpired(expirationTime) {
  return (
    expirationTime === null ||
    Date.now() >= expirationTime - TOKEN_EXPIRATION_BUFFER
  );
}
...
async function run() {
  ...
  for (const account of accounts) {
    ...
    if (
      existingTokenData &&
      !isTokenExpired(existingTokenData.expirationTime)
    ) {
      // Use existing token
    ...
    while (true) {
      ...
      try {
        ...
        if (data.gameId) {
          ...
          console.log(
            `Waiting ${randomWaitTime} seconds before calling API 2...`
          );
        ...
        console.error(
          `\x1b[31mError in account ${name}:\x1b[0m`,
          error.message
        ); // Red color for error
